    ...
    async def remindme(
        ...
        **Examples:**
        ...
        message_or_text = message_or_text or (
            ctx.message.reference.cached_message if getattr(ctx.message, "reference", None) is not None else None
        )
        if isinstance(message_or_text, discord.Message):
            content = {
                ...
                "files": {
                    attachment.filename: attachment.url for attachment in getattr(ctx.message, "attachments", [])
                },
            }
        ...
        reminder = await self.create_reminder(
            ...
            content=content,
            jump_url=ctx.message.jump_url if not getattr(ctx, "interaction", None) else f"https://discord.com/channels/{ctx.guild.id if ctx.guild else '@me'}/{ctx.channel.id}/{ctx.interaction.id}",
            created_at=utc_now,
            expires_at=expires_at,
            repeat=repeat,
        )
        ...
        message = await ctx.send(
            ...
            view=view,
            reference=ctx.message.to_reference(fail_if_not_exists=False) if not getattr(ctx, "interaction", None) else None,  # discord.MessageReference.from_message(ctx.message, fail_if_not_exists=False)
