...
# ----------------------------------------------------------------
    ...
    async def captchaset_welcometitle(self, ctx: commands.Context, *, title: str):
        ...
        if len(title) > 256:
            return await ctx.send("❌ Error: Title exceeds the Discord limit of 256 characters.")
    ...
    async def captchaset_welcomedesc(self, ctx: commands.Context, *, description: str):
        ...
        if len(description) > 4096:
            return await ctx.send("❌ Error: Description exceeds the Discord limit of 4096 characters.")
    ...
    async def challenge_add(self, ctx: commands.Context, challenge_id: str, image_url: str, correct_option: str, *, options: str):
        ...
        if not image_url.startswith(("http://", "https://")):
            return await ctx.send("❌ Security Error: `image_url` must use HTTP or HTTPS.")
        if len(image_url) > 2048:
            return await ctx.send("❌ Error: `image_url` exceeds the Discord limit of 2048 characters.")

        options_list = [o.strip() for o in options.split(',')]
        if any(len(o) > 80 for o in options_list):
            return await ctx.send("❌ Error: An option exceeds the Discord limit of 80 characters for button labels.")
