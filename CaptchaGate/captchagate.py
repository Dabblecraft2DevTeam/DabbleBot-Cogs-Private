...
# ----------------------------------------------------------------
    ...
    async def captchaset_welcometitle(self, ctx: commands.Context, *, title: str):
        ...
        if len(title) > 256:
            return await ctx.send("❌ Title cannot exceed 256 characters (Discord limit).")
    ...
    async def captchaset_welcomedesc(self, ctx: commands.Context, *, description: str):
        ...
        if len(description) > 4096:
            return await ctx.send("❌ Description cannot exceed 4096 characters (Discord limit).")
    ...
    async def challenge_add(self, ctx: commands.Context, challenge_id: str, image_url: str, correct_option: str, *, options: str):
        ...
        if len(image_url) > 2048:
            return await ctx.send("❌ Image URL cannot exceed 2048 characters (Discord limit).")

        options_list = [o.strip() for o in options.split(',')]
        
        for opt in options_list:
            if len(opt) > 80:
                return await ctx.send(f"❌ Option '{opt}' exceeds the 80 character limit for button labels.")
