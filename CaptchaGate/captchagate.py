...
# ----------------------------------------------------------------
    ...
    async def captchaset_welcometitle(self, ctx: commands.Context, *, title: str):
        ...
        if len(title) > 256:
            return await ctx.send("❌ The embed title cannot exceed 256 characters due to Discord limits.")
    ...
    async def captchaset_welcomedesc(self, ctx: commands.Context, *, description: str):
        ...
        if len(description) > 4096:
            return await ctx.send("❌ The embed description cannot exceed 4096 characters due to Discord limits.")
    ...
    async def challenge_add(self, ctx: commands.Context, challenge_id: str, image_url: str, correct_option: str, *, options: str):
        ...
        if len(image_url) > 2048:
            return await ctx.send("❌ The image URL cannot exceed 2048 characters due to Discord limits.")

        options_list = [o.strip() for o in options.split(',')]
        
        if any(len(o) > 80 for o in options_list):
            return await ctx.send("❌ Each option label cannot exceed 80 characters due to Discord button limits.")
