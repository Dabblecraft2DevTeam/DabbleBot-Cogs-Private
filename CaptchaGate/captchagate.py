...
# ----------------------------------------------------------------
    ...
    async def challenge_add(self, ctx: commands.Context, challenge_id: str, image_url: str, correct_option: str, *, options: str):
        ...
        """
        # Security: Validate image_url to prevent SSRF and Discord API limit errors (max 2048 chars)
        if not (image_url.startswith("http://") or image_url.startswith("https://")):
            return await ctx.send("❌ Security Error: Invalid `image_url`. The URL must start with `http://` or `https://`.")
        if len(image_url) > 2048:
            return await ctx.send("❌ Security Error: The `image_url` must be 2048 characters or fewer.")
