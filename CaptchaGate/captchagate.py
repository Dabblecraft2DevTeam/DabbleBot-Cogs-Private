...
# ----------------------------------------------------------------
    ...
    async def challenge_add(self, ctx: commands.Context, challenge_id: str, image_url: str, correct_option: str, *, options: str):
        ...
        """
        # Security Validation: URL Scheme and Length check to prevent SSRF and Self-DoS
        if not image_url.startswith(("http://", "https://")):
            return await ctx.send("❌ The `image_url` must start with `http://` or `https://`.")
        if len(image_url) > 2048:
            return await ctx.send("❌ The `image_url` exceeds Discord's maximum limit of 2048 characters.")
