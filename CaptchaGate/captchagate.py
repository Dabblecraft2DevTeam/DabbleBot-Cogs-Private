...
# ----------------------------------------------------------------
    ...
    async def challenge_add(self, ctx: commands.Context, challenge_id: str, image_url: str, correct_option: str, *, options: str):
        ...
        """
        if len(image_url) > 2048 or not image_url.startswith(("http://", "https://")):
            return await ctx.send("❌ Invalid image URL. It must start with http/https and be under 2048 characters.")

        options_list = [o.strip() for o in options.split(',')]
        
        for opt in options_list:
            if len(opt) > 80:
                return await ctx.send(f"❌ Option `{opt}` exceeds the 80 character limit for buttons.")
