...
# ----------------------------------------------------------------
    ...
    async def captchaset_welcometitle(self, ctx: commands.Context, *, title: str):
        ...
        if len(title) > 256:
            return await ctx.send("❌ Title is too long. Discord allows a maximum of 256 characters for an embed title.")
    ...
    async def captchaset_welcomedesc(self, ctx: commands.Context, *, description: str):
        ...
        if len(description) > 4096:
            return await ctx.send("❌ Description is too long. Discord allows a maximum of 4096 characters for an embed description.")
    ...
    async def challenge_add(self, ctx: commands.Context, challenge_id: str, image_url: str, correct_option: str, *, options: str):
        ...
        if len(image_url) > 2048:
            return await ctx.send("❌ The `image_url` is too long. Discord allows a maximum of 2048 characters for an image URL.")

        options_list = [o.strip() for o in options.split(',')]
        
        for option in options_list:
            if len(option) > 80:
                return await ctx.send(f"❌ Option `{option}` is too long. Discord allows a maximum of 80 characters for a button label.")
