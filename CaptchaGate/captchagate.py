...
class CaptchaGate(commands.Cog):
    ...
    def __init__(self, bot: Red):
        ...
        # Tracks {(guild_id, challenge_id): float} for least-recently-used challenge selection
        self._challenge_last_used = {}
...
# ----------------------------------------------------------------
    ...
    async def _send_captcha(self, member: discord.Member):
        ...
        # Using in-memory cache to prevent I/O bottleneck
        sorted_challenges = sorted(
            challenge_items,
            key=lambda item: self._challenge_last_used.get((member.guild.id, item[0]), item[1].get("last_used", 0))
        )
        ...
        # Mark the selected challenge as used in memory instead of writing to config
        # to prevent Self-DoS via I/O bound database updates in high frequency join events
        self._challenge_last_used[(member.guild.id, selected_challenge_id)] = time.time()
...
# ----------------------------------------------------------------
    ...
    async def captchaset_welcometitle(self, ctx: commands.Context, *, title: str):
        ...
        if len(title) > 256:
            return await ctx.send("❌ Title cannot exceed 256 characters.")
    ...
    async def captchaset_welcomedesc(self, ctx: commands.Context, *, description: str):
        ...
        if len(description) > 4096:
            return await ctx.send("❌ Description cannot exceed 4096 characters.")
    ...
    async def challenge_add(self, ctx: commands.Context, challenge_id: str, image_url: str, correct_option: str, *, options: str):
        ...
        if any(len(o) > 80 for o in options_list):
            return await ctx.send("❌ Each option must be 80 characters or fewer due to Discord button limits.")
    ...
    async def challenge_list(self, ctx: commands.Context):
        ...
        for cid, c in challenges.items():
            ...
            last_used_ts = self._challenge_last_used.get((ctx.guild.id, cid), c.get("last_used", 0))
