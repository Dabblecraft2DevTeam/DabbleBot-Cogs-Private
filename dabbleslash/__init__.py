...
async def _setup(bot: Red):
    ...
    assert bot.user
    # Use configurable short name, defaulting to "db"
    config = bot.get_cog("OneTrueSlash")
    command_name = "db"
    try:
        onetrueslash.name = valid_app_name(command_name)
        bot.tree.add_command(onetrueslash, guild=None)
    except ValueError:
        await bot.send_to_owners(
            f"`onetrueslash` was unable to use the name {command_name!r} "
            "as a valid slash command name. The command name was left unchanged."
        )
