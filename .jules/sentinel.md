...
## 2024-05-27 - [Missing Embed Validation Self-DoS]
**Vulnerability:** Bot self-DoS when adding challenges and welcoming users due to lack of character limit validations on input which are later embedded (titles, descriptions, image urls, and option buttons). Discord API strictly enforces bounds (256, 4096, 2048, and 80 characters respectively). Exceeding these bounds causes HTTP 400 Bad Request which breaks the verification workflow.
**Learning:** We need to explicitly bound-check user input when configuring bot data that will be passed into discord embeds and views to prevent bot-crashing logic errors.
**Prevention:** Always add explicit length validation checking Discord API limitations before persisting configuration data to the RedBot config.
