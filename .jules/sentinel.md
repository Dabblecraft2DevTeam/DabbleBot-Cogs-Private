...
## 2026-06-08 - SSRF and Discord Embed Self-DoS Prevention
**Vulnerability:** Unvalidated URL input in the `challenge_add` command of CaptchaGate allowed potential Server-Side Request Forgery (SSRF) via Discord's media proxy. Additionally, missing length validation on `welcometitle` and `welcomedesc` could cause Self-DoS when creating embeds, resulting in Discord API 400 Bad Request errors.
**Learning:** Discord embeds have strict length constraints (256 for titles, 4096 for descriptions) and require valid URL schemes for media fields to prevent abuse or API errors.
**Prevention:** Always validate URL input for correct schemes (http/https) and enforce strict length bounds corresponding to Discord's API limits before rendering dynamic content into embeds.
