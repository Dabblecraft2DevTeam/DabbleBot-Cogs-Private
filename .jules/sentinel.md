...
**Vulnerability:** Discord API character limits (256 for embed titles, 4096 for embed descriptions, 2048 for URLs, 80 for button labels) were not enforced on user-provided input before constructing `discord.Embed` and `discord.ui.Button` instances.
**Learning:** If a bot attempts to send an embed or view that exceeds Discord's strict character limits, the API responds with a 400 Bad Request error. When this happens during critical flows (like CAPTCHA verification), it acts as a Self-Denial of Service (Self-DoS), breaking the application's functionality.
**Prevention:** Always validate the length of user-provided strings against Discord's known API limits *before* they are used in embeds, views, or messages.
