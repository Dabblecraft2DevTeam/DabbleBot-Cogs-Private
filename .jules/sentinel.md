...
## 2026-05-09 - Missing Embed Validation (Self-DoS)
**Vulnerability:** Discord API character limits (256 for title, 4096 for description, 2048 for URL, 80 for button labels) were not validated before being passed to embed and view creation in the CaptchaGate cog.
**Learning:** If user-provided input is not validated against Discord API strict character limits before being used in embeds or views, it leads to a 400 Bad Request error. This causes the bot command to fail or crash, creating a Self-DoS vulnerability.
**Prevention:** Always check the length of strings provided by users against the Discord API documentation limits before passing them into `discord.Embed` fields, URLs, or `discord.ui.Button` labels.
