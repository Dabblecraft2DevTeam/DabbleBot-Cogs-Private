...
class CommandStats(commands.Cog):
    ...
    async def on_app_command_completion(self, interaction, command):
        """Record native slash command events (not DabbleSlash — those go through on_command_completion)."""
        if interaction.user.bot:
            return
        # DabbleSlash pipes through Red's command pipeline, so it already
        # triggers on_command_completion with the real command name.
        # This listener catches native app commands from other cogs.
        name = command.qualified_name
        guild = interaction.guild
        self.record(name, guild, False)
